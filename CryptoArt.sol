...
contract Cryptoart is ERC721, Ownable {
	...
	function createToken(address buyer, uint256 tokenId) private {
		_mint(buyer, tokenId);
	}
	...
	function _buyToken(
		...
		if (y.tokenOwner == y.tokenCreator) {
			fee = SafeMath.div(SafeMath.mul(y.tokenSellPrice, percentageCut), 100);
			...
			creatorfee = SafeMath.div(fee, 2);

			priceAfterFee = SafeMath.sub(y.tokenSellPrice, fee);
			y.tokenCreator.transfer(creatorfee);
			y.tokenOwner.transfer(priceAfterFee);
			totalbal = SafeMath.add(SafeMath.div(fee, 2), totalbal);
		}
		uint index;
