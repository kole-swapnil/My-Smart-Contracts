...
abstract contract ERC20Interface {
	...
	function balanceOf(address tokenOwner) public view virtual returns (uint balance); //"constant" deprecated at 0.5.0
}
...
contract SuperWorldToken is ERC721 {
	...
	//changed
	function getTokenId(string memory lon, string memory lat) public pure returns (uint) {
	...
	function buyToken(string memory lon, string memory lat) public payable returns (bool) {
		uint tokenId = getTokenId(lon, lat);
	...
	//changed
	function truncateDecimals(string memory str, uint decimal) public pure returns (string memory) {
	...
	function indexOfComma(string memory str) private pure returns (uint) {
		bytes memory strBytes = bytes(str);
		uint length = strBytes.length;
	...
	//changed
	function bytesToString(bytes memory _dataBytes) private pure returns (string memory) {
	...
	//changed
	function bytes32ToString(bytes32 _dataBytes32) private pure returns (string memory) {
		bytes memory bytesString = new bytes(32);
		uint charCount = 0;
		uint j;
		for (j = 0; j < 32; j++) {
			//outscope declaration
			bytes1 char = bytes1(bytes32(uint(_dataBytes32) * 2**(8 * j)));
			if (char != 0) {
