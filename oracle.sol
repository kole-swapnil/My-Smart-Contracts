...
contract ExampleContract is usingProvable {
	...
	function getTokenId(string memory long, string memory lati) public pure returns (uint) {
	...
	function updatePrice(string a, string b) payable {
		if (provable_getPrice("URL") > this.balance) {
			...
			lat = string(
				abi.encodePacked(
					"json(http://geo.superworldapp.com/api/json/token/get?tokenId=",
					a,
					"&blockchain=e&networkId=",
					b,
					").data.lon"
				)
			);
